...
/* Given the payload size of the block */
...
static void *find_fit(size_t asize)
{
  ...
  /* best fit search */
  void *bp;
  char *best = heap_listp;
  size_t leftover = ~0x0;
  
  /* search the entire list to find the free block that gives minimum leftover */
  for (bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp)) {
    if (!GET_ALLOC(HDRP(bp)) && (asize <= GET_SIZE(HDRP(bp)))) {
    	size_t curr_leftover = PAYLOAD_SIZE(bp) - asize;
